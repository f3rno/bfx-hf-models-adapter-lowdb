...
/**
 * Bulk inserts records, updating them if they already exist.
 *
 * **UNIQUE TO LOWDB!**
 *
 * @throws {Error} fails if the model is lacking an index
 ...
 * @param {object[]} docs - records to bulk upsert
 * @returns {object[]} insertedRecords
 */
const bulkUpsert = async (db, { path, index, indexMatches }, docs) => {
  if (!index) {
